#####################################################
### SCRIPT TO PERFORM POSTERIOR PREDICTIVE CHECKS ###
#####################################################
...
# prepare functions
pp_check_beta_JSDM <- function(mod,Y_mat,X,sp_name_list,n_rep=50,bin_width = 0.05,test_quantities=FALSE,rho_modeled=FALSE,C=1000,right_censoring=FALSE,a=1,b=0.5,min_rho=0,save_plot=FALSE,save_loc="",im_width=800,im_height=600) {
...
pp_check_beta_spat_JSDM <- function(mod,Y_mat,X,sp_name_list,P,n_rep,bin_width=0.05, test_quantities = FALSE, rho_modeled = FALSE, C=1000, right_censoring = FALSE, a = 1,b = 0.5, min_rho=0,save_plot=FALSE,save_loc="",im_width=800,im_height=600) {
...
pp_check_2stage <- function(mod,Y_mat,X,sp_name_list,n_rep=50,bin_width = 0.05,test_quantities=FALSE,save_plot=FALSE,save_loc="",im_width=800,im_height=600) {
  ### FUNCTION TO PERFORM POSTERIOR PREDICTIVE CHECKS FOR MULTIVARIATE LEFT-CENSORED BETA REGRESSION MODEL ###
  ### INPUTS: ###
  # mod: stanfit object
  # Y_mat: observations (NxJ matrix)
  ...
  for (i in 1:n_rep) {
    ...
    # calculate latent f 
    f <- alpha + X %*% beta + Z %*% lambda # n x J matrix
...
pp_check_2stage_spat <- function(mod,Y_mat,X,sp_name_list,P,n_rep,bin_width=0.05, test_quantities = FALSE, save_plot=FALSE,save_loc="",im_width=800,im_height=600) {
  ### FUNCTION TO PERFORM POSTERIOR PREDICTIVE CHECKS FOR LEFT-CENSORED BETA REGRESSION MODEL ###
  ### INPUTS: ###
  # mod: stanfit object
  # Y_mat: observations (NxJ matrix)
  ...
  for (i in 1:n_rep) {
    ...
    phi_M <- post.samples$phi_M[idx,]
    
    f_M <- alpha_M + X %*% beta_M + P %*% phi_M
    mu_M <- exp(f_M)
    ...
    # calculate latent f 
    f <- alpha + X %*% beta + (Z + P %*% phi) %*% lambda # n x J matrix
...
plot_community_checks <- function(y_rep_list, Y_mat, plot_richness = FALSE, plot_euclidian_distances = FALSE, test_quantities = FALSE, bin_width = 0.05) {
  ...
  mean_eucl_dists <- c() # rooted mean euclidian distances sqrt(1/N * sum_i(||y_i - ỹ_i||))
  ...
  for (i in 1:n_rep) {
    ...
    mean_eucl_dists <- c(mean_eucl_dists, sqrt(mean(eucl_dists)))
  ...
  # plot histograms of test quantities (if asked)
  if (test_quantities) {
    par(mfrow = c(2,2),
        mar = c(2,4,2,0))
  ...
  if(plot_euclidian_distances) {
    par(mfrow = c(1,1))
    hist(mean_eucl_dists, breaks = 40, main = "sqrt(1/N*sum_i(||y_i - ỹ_i||)", xlab = "||y_i - ỹ_i||")
    
    ### draw also a red vertical line for the mean
    avg_mean_eucl_dist <- median(mean_eucl_dists)
...
# perform PP-checks
n_rep <- 200 #number of replicated datasets
...
dev.off()
png(paste0("plots/final_results/SSDM/",subfolder,"/M1/pp_checks/community_pp_check_replicates.png"), width = im_width, height = im_height)
...
dev.off()
png(paste0("plots/final_results/SSDM/",subfolder,"/M3/pp_checks/community_pp_check_replicates.png"), width = im_width, height = im_height)
...
dev.off()
png(paste0("plots/final_results/JSDM/",subfolder,"/M1/pp_checks/community_pp_check_replicates.png"), width = im_width, height = im_height)
...
plot_community_checks(Y_rep_list_JSDM,Y/100, plot_richness = FALSE,plot_euclidian_distances = FALSE, test_quantities = TRUE)
dev.off()
png(paste0("plots/final_results/JSDM/",subfolder,"/M3/pp_checks/community_pp_check_replicates.png"), width = im_width, height = im_height)
plot_community_checks(Y_rep_list_JSDM,Y/100, plot_richness = FALSE,plot_euclidian_distances = FALSE, test_quantities = FALSE)
dev.off()
...
dev.off()
png(paste0("plots/final_results/two_stage/",subfolder,"/M1/pp_checks/community_pp_check_replicates.png"), width = im_width, height = im_height)
...
dev.off()
png(paste0("plots/final_results/two_stage/",subfolder,"/M3/pp_checks/community_pp_check_replicates.png"), width = im_width, height = im_height)
