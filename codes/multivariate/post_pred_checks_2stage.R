########################################################################
### SCRIPT TO PERFORM POSTERIOR PREDICTIVE CHECKS FOR 2-STAGE MODELS ###
########################################################################
...
pp_check_2stage <- function(mod,Y_mat,X,sp_name_list,n_rep=50,bin_width = 0.05,test_quantities=FALSE) {
...
### pp-checks with JSDM
set.seed(123)
mod.2stage <- readRDS("models/two_stage/n_500/M1/JSDM_NegBin_DirMult.RDS")
Y_rep_list_2stage <- pp_check_2stage(mod.2stage,Y,X.sec_ord,sp_names,100,5,test_quantities = TRUE)

set.seed(42)
plot_community_checks(Y_rep_list_2stage,Y,plot_richness = FALSE,plot_euclidian_distances = FALSE,test_quantities = TRUE,bin_width = 5)

pp_check_2stage_spat <- function(mod,Y_mat,X,sp_name_list,P,n_rep,bin_width=0.05, test_quantities = FALSE) {
...
### pp-checks with JSDM
set.seed(123)
mod.2stage_20_species.spat <- readRDS("models/two_stage/n_100/M3/JSDM_NegBin_DirMult_spatial.RDS")
Y_rep_list_2stage.spat <- pp_check_2stage_spat(mod.2stage_20_species.spat,Y,X.sec_ord,sp_names,P,100,5,test_quantities = TRUE)

set.seed(42)
plot_community_checks(Y_rep_list_2stage.spat,Y,plot_richness = FALSE,plot_euclidian_distances = FALSE,test_quantities = TRUE,bin_width = 5)
