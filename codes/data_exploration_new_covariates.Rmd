...
2020-2021 combined with $n=1000$
...
rand_idx <- sample(1:nrow(df.vect.sub),n)
...
### save the data
save(df_sub, file = "../data/estonia_new/train_2020_2021_n1000.Rdata")
for (var in vars) {
  hist(df_sub[,var],breaks=40, xlab = "value", main = var, probability = TRUE)
}
...
r <- rast(extent = ext_rast, crs = "EPSG:3035", res = res) ### which coordinate system is this?
depth.rast <- rasterize(as.matrix(depth[,c("x","y")]),r,values=depth$depth.mean)

# ### are the centroids what I want them to be: same as in the data frame?
# depth_centroids <- centroids(r)
...
### bring in predictive grid
grid_raster <- vect("../data/estonia_new/spatial_random_effect_grid.shp")
...
plot(depth.rast.eur)
plot(europe.vect.cut, add = TRUE, col = "grey")
...
grid_points_vect <- terra::subset(grid_points_vect, id_copernicus_phy_bgc %in% unique(grid_2021_july$id_copernicus_phy_bgc), NSE = T)
