...
### save for remote computer which do not have terra
save(P,observed_grid_cells.df,file = "data/estonia_new/for_remote_computer/P_and_coords_n2000")
...
for (model_subfolder in c("")) {
  ...
  for (sp_name in sp_names[c(4,5,8,12)]) {
    ...
    mod.ZIBeta_spat <- stan(stan_file_loc,
                            data = dat.beta_spat, chains = n_chains, iter = n_iter, seed = 42,
                            pars = c("mu","prob_suit","z","z_pi"), include = FALSE)
...
vals <- colMeans(pred_list_m4$EY_sam)
vars <- apply(pred_list_m4$EY_sam,2,var)
...
plot(r, colNA = "lightgrey", main = "M4 (w/ ZI,  w/ RE)",plg = list(title = "E[coverage]"),
     xlab = "Easting (m)", ylab = "Northing (m)")
#plot(r.scaled, colNA = "lightgrey", main = "Amphi rel. exp. coverage: M3 (w/o ZI,  w/ RE)")
plot(r.hotspot, colNA = "lightgrey", main = "M4 (w/ ZI, w/ RE)", col = c("red","blue"),
     plg = list(title = "hotspot", legend = c("no","yes")),
     xlab = "Easting (m)", ylab = "Northing (m)")

# plot variance
plot(r.vars,colNA="lightgrey", main = "Amphi variance: M4 (w/ ZI, w/ RE)")
...
### for mu
locs <- pred_grid_1km_2021_july_df[,c("x","y")]
vals <- colMeans(pred_list_m4$phi_mu_pred_sam)
...
par(mfrow = c(1,1))
plot(r.phi, colNA = "lightgrey", main = "Amhpi spatial REs (mu): M4 (w/o ZI, w/ RE)")

### for pi
locs <- pred_grid_1km_2021_july_df[,c("x","y")]
vals <- colMeans(pred_list_m4$phi_pi_pred_sam)
test_df <- as.data.frame(cbind(locs, vals))
...
par(mfrow = c(1,1))
plot(r.phi, colNA = "lightgrey", main = "Amhpi spatial REs (pi): M4 (w/o ZI, w/ RE)")
