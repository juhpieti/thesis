...
model {
  ...
  //alpha_pi ~ normal(-0.4,sqrt(0.25)); // 2nd option
  alpha_pi ~ normal(-1,sqrt(0.2));
  
  //rho ~ cauchy(0,sqrt(10));
  1/rho ~ student_t(4,0,0.1);
  target += -2*log(rho); // adjust for log of Jacobian
  ...
  for (i in 1:N) {
    ...
    real mu_i;
    mu_i = mu[i];
    mu_i = fmin(fmax(mu_i,1e-08), 1 - 1e-08); // restrict to open (0,1)
    
    // calculate the probability of suitability
    real pi_i;
    pi_i = prob_suit[i];
    pi_i = fmin(fmax(pi_i,1e-08), 1 - 1e-08); // restrict to open (0,1)
