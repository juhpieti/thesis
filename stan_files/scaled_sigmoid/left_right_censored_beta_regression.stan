...
data {
  ...
  real<lower=0> b; // beta is extended to (-a,1+b)
}
...
model {
  ...
  for (i in 1:N) {
    ...
    if (y[i] == 0) {
      ...
      target += log(fmax(1e-20, beta_cdf(a/(a+b+1) | mu_i*rho_i, (1-mu_i)*rho_i)));
      
    // since Beta-distribution has support on open interval (0,1), take y = 1 cases also separately, substract a small value
    } else if (y[i] ==  1) {
      target += log(fmax(1e-20, 1-beta_cdf((a+1)/(a+b+1) | mu_i*rho_i, (1-mu_i)*rho_i)));
      //target += log(1-beta_cdf((a+1)/(a+b+1) | mu_i*rho_i, (1-mu_i)*rho_i));
      //(y[i]-eps+a)/(a+1) ~ beta(mu_i*rho_i, (1-mu_i)*rho_i);
      
    // y \in (0,1)  
    } else {
      (y[i]+a)/(a+b+1) ~ beta(mu_i*rho_i, (1-mu_i)*rho_i); // scale to [0,1] interval
    }
  }
}
...
generated quantities {
  ...
  for (i in 1:N) {
    ...
    if (y[i] == 0) {
      ...
      log_lik[i] = log(fmax(1e-20, beta_cdf(a/(a+b+1) | mu_i*rho_i, (1-mu_i)*rho_i)));
    } else if (y[i]==1) {
      log_lik[i] = log(fmax(1e-20, 1-beta_cdf((a+1)/(a+b+1) | mu_i*rho_i, (1-mu_i)*rho_i)));
      //log_lik[i] = log(1-beta_cdf((a+1)/(a+b+1) | mu_i*rho_i, (1-mu_i)*rho_i));
      //log_lik[i] = beta_lpdf((y[i]-eps+a)/(a+1) | mu_i*rho_i, (1-mu_i)*rho_i) - log(1+a);
    } else {
      log_lik[i] = beta_lpdf((y[i]+a)/(a+b+1) | mu_i*rho_i, (1-mu_i)*rho_i) - log(1+a+b); // scale to [0,1] interval
    }
  }
}
