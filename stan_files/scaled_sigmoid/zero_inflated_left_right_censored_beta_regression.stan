...
model {
  ...
  for (i in 1:N) {
    ...
    if (y[i] == 0) {
      ...
      target += log_sum_exp(log(1-pi_i), log(pi_i) + log(fmax(1e-20, beta_cdf(a/(a+b+1) | mu_i*rho_i, (1-mu_i)*rho_i))));
    ...
    // y \in (0,1)
      ...
      (y[i]+a)/(a+b+1) ~ beta(mu_i*rho_i, (1-mu_i)*rho_i);
      target += log(pi_i);
    }
  }
}
...
generated quantities {
  ...
  for (i in 1:N) {
    ...
    if (y[i] == 0) {
      ...
      log_lik[i] = log_sum_exp(log(1-pi_i), log(pi_i) + log(fmax(1e-20, beta_cdf(a/(a+b+1) | mu_i*rho_i, (1-mu_i)*rho_i))));
    } else if (y[i] == 1) {
      log_lik[i] = log(fmax(1e-20, 1-beta_cdf((a+1)/(a+b+1) | mu_i*rho_i, (1-mu_i)*rho_i))) + log(pi_i);
    } else {
      log_lik[i] = beta_lpdf((y[i]+a)/(a+b+1) | mu_i*rho_i, (1-mu_i)*rho_i) - log(1+a+b) + log(pi_i); // scale to [0,1] interval
    }
  }
}
