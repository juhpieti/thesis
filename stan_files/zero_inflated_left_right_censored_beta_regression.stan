...
model {
  ...
  for (i in 1:N) {
    ...
    if (y[i] == 0) {
      ...
      target += log_sum_exp(log(1-pi_i), log(pi_i) + log(fmax(1e-20, beta_cdf(a/(a+b+1) | mu_i*rho, (1-mu_i)*rho))));
      
    } else if (y[i]==1) { // stan declares beta on open interval (0,1)
      //(y[i]-eps+a)/(a+1) ~ beta(mu_i*rho, (1-mu_i)*rho);
      target += log(fmax(1e-20, 1-beta_cdf((a+1)/(a+b+1) | mu_i*rho, (1-mu_i)*rho)));
      target += log(pi_i);
      
    // y \in (0,1)
    } else {
      (y[i]+a)/(a+b+1) ~ beta(mu_i*rho, (1-mu_i)*rho);
      target += log(pi_i);
    }
  }
}
...
generated quantities {
  ...
  for (i in 1:N) {
    ...
    if (y[i] == 0) {
      ...
      log_lik[i] = log_sum_exp(log(1-pi_i), log(pi_i) + log(fmax(1e-20, beta_cdf(a/(a+b+1) | mu_i*rho, (1-mu_i)*rho))));
    } else if (y[i] == 1) {
      //log_lik[i] = beta_lpdf((y[i]-eps+a)/(a+1) | mu_i*rho, (1-mu_i)*rho) - log(1+a) + log(pi_i);
      log_lik[i] = log(fmax(1e-20, 1-beta_cdf((a+1)/(a+b+1) | mu_i*rho, (1-mu_i)*rho))) + log(pi_i);
    } else {
      log_lik[i] = beta_lpdf((y[i]+a)/(a+b+1) | mu_i*rho, (1-mu_i)*rho) - log(1+a+b) + log(pi_i); // scale to [0,1] interval
    }
  }
}
