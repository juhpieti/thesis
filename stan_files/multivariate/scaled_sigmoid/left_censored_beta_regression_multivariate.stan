...
parameters {
  ...
  matrix[n_var/2,J] beta_rho_1;
  matrix[n_var/2,J] beta_rho_2;
...
transformed parameters{
  ...
  matrix[N,J] Rho; // gather precision parameters
  for (n in 1:N) {
    Rho[n] = C*inv_logit(to_row_vector(alpha_rho) + X[n]*append_row(beta_rho_1,beta_rho_2));
  }
}
...
model {
  ...
  to_vector(beta_rho_1) ~ normal(0,sqrt(1));
  to_vector(beta_rho_2) ~ normal(0,sqrt(1));
  ...
  for (i in 1:N) {
    for (j in 1:J) {
      target += lc_beta_density(Y[i,j],Mu[i,j],Rho[i,j],a,eps,N);
    }
  }
}
...
generated quantities {
  ...
  for (i in 1:N) {
    real ll = 0;
    for (j in 1:J) {
      ll += lc_beta_density(Y[i,j],Mu[i,j],Rho[i,j],a,eps,N);
    }
    log_lik[i] = ll;
  }
}
