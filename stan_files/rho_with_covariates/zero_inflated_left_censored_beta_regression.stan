...
model {
  ...
  beta_rho_1 ~ normal(0,sqrt(1));
  beta_rho_2 ~ normal(0,sqrt(1));
  
  alpha ~ normal(0,sqrt(2)); // ORIGINAL
  //alpha_pi ~ normal(0,sqrt(2)); // ORIGINAL
  alpha_pi ~ normal(-0.4,sqrt(0.25));
  //rho ~ cauchy(0,sqrt(10));
  alpha_rho ~ normal(0,sqrt(1));
  ...
  for (i in 1:N) {
    ...
    //rho_i = rho;
    rho_i = rho[i];
    rho_i = fmin(fmax(rho_i,1e-08),1e06); // prevent exact zero
...
generated quantities {
  ...
  for (i in 1:N) {
    ...
    //rho_i = rho;
    rho_i = rho[i];
    rho_i = fmin(fmax(rho_i,1e-08),1e06);
    
    // likelihood terms
    if (y[i] == 0) {
      log_lik[i] = log_sum_exp(log(1-pi_i), log(pi_i) + beta_lcdf(a/(a+1) | mu_i*rho_i, (1-mu_i)*rho_i));
    } else if (y[i] == 1) {
      log_lik[i] = beta_lpdf((y[i]-1e-08+a)/(a+1) | mu_i*rho_i, (1-mu_i)*rho_i) - log(1+a) + log(pi_i);
      //log_lik[i] += log(pi_i);
    } else {
